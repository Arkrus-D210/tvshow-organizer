...
def main():
    file_list = []
    #Generate inventory
    t = PrettyTable(["File","Title","Season","Source Directory","Target Directory","Season Directory Exists"])
            ...
            for name in files:
                for media in ACCEPTED_TYPES:
                    if name.endswith(media) and "sample" not in name.lower():
                        ...
                        season_directory = os.path.isdir(f"{target_directory}\\{season}")
                        payload = \
                            {
                                "title": title,
                                "season": season,
                        ...
                        t.add_row([name,title,season,source_directory,target_directory,season_directory])
        ...
        for file in file_list:
            ...
            if file["title"] != None:           
                ...
                if not file["season_directory"]:
                    ...
                    print(f"With a full path of {file['target_directory']}\\{file['season']}")
                    season_directory = os.path.isdir(f"{file['target_directory']}\\{file['season']}")
                    if season_directory == False:
                        os.mkdir(f"{file['target_directory']}\\{file['season']}")
                        log_discord(f"{file['target_directory']}\\{file['season']} has been created")
                #move file
                print(f"Moving {file['filename']}")
                shutil.move(file["source_directory"],f"{file['target_directory']}\\{file['season']}\\{file['filename']}")
                print(f"{file['filename']} Moved")
            else:
                print(f"Moving {file['filename']}")
                shutil.move(file["source_directory"],f"{UNSORTED_DRIVE[0]}\\{file['filename']}")
                print(f"{file['filename']} Moved")
